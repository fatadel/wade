<template>
  <div class="performance-container">
    <oSelection
      ...
      :showButtons="'selection-btn-reset'"
...
export default Vue.extend({
  ...
  methods: {
    ...
    startPerformancePrediction(settings: WADE.PerformanceMeasurementSettings) {
      ...
      this.resultData = (this as any)
        .getPerformancePrediction(settings)
        .then((res) => {
    ...
    saveMeasurement(measurements) {
      ...
      for (let i = 0, l = measurements.length; i < l; i++) {
        const key =
          `${measurements[i].name.replace(/\s/g, "")}` +
    ...
    saveTD(measurements) {
      ...
      const storage = require("electron-json-storage");
      const annotatedTD = new TdAnnotater(
        (this as any).getSavedTd((this as any).$route.params.id),
        measurements
      ).getAnnotatedTD();
      const name = `${
        (this as any).$route.params.id
      }_annotated_${getCurrentDate()}.json`;
    ...
    selectionResetted() {
      ...
      this.resultData = new Promise((rej, res) => {
        rej([]);
        res([]);
      });
    },
