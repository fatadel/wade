...
export default {
  ...
  state: {
    ...
    tdEditorPlaceholder:
      "Paste your Thing Description here or press the upload button."
  },
  actions: {
    async processChangedTd({ commit, state }, payload: any) {
      ...
      if (!payload.td || payload.td.length <= 0) {
        commit(
          "setTdState",
          payload.tdState ? payload.tdState : TdStateEnum.NO_TD
        );
      ...
      const parsedTd = await Api.consumeAndParseTd(
        payload.td,
        payload.config,
        payload.protocols
      );
      ...
      if (
        ...
        const hasInteractions =
          state.tdParsed.propertyInteractions.length > 0 ||
    ...
    async removeFromSelectedInteractions({ commit, state }, payload) {
      ...
      selectedInteractions.splice(
        selectedInteractions.indexOf(payload.interactionToRemove),
        1
      );
      commit("setSelections", selectedInteractions);
      commit("setStatusMessage");
  ...
  mutations: {
    ...
    setStatusMessage(state: any) {
      state.statusMessage = Api.updateStatusMessage(
        state.tdState,
        state.errorMsg,
    ...
    setActiveTab(
      state: any,
      payload: { tabbarKey: string; activeTab: string }
    ) {
  ...
  getters: {
    ...
    isValidTd(state: any) {
      return (
        state.tdState === TdStateEnum.VALID_TD ||
